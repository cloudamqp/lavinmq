...
module LavinMQ
  module Clustering
    ...
    class Server
      ...
      # Commit the current in-sync set to the coordinator. Returns whether it
      # committed: false means the write was rejected (a deposed leader, or
      # raft leadership lost/overwritten), so @dirty_isr stays set and callers
      # retry rather than acknowledge against a stale ISR.
