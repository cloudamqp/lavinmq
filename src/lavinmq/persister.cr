...
module LavinMQ
  ...
  # publishes (mark_dirty); sync then msyncs exactly those files, so a publish
  # confirm doesn't also write out dirty pages of unrelated files (ack files,
  # other vhosts) the way a whole-filesystem syncfs did.
  class Persister
