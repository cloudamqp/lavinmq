...
module LavinMQ
  module MQTT
    class Brokers
      ...
      def initialize(@vhosts : VHostStore, @permission_service : Auth::PermissionService)
      ...
      private def populate
        @vhosts.each do |(name, vhost)|
          @brokers[name] = Broker.new(vhost, @permission_service)
        end
      end
      ...
      def on(event : VHostStore::Event, data : Object?)
        ...
        in VHostStore::Event::Added
          @brokers[vhost] = Broker.new(@vhosts[vhost], @permission_service)
