...
module LavinMQ
  module MQTT
    class SubscriptionTree
      @wildcard_rest = Hash(Session, UInt8).new
      @plus : SubscriptionTree?
      @leafs = Hash(Session, UInt8).new
      ...
      @non_wildcards = Hash(String, Hash(Session, UInt8)).new do |h, k|
        h[k] = Hash(Session, UInt8).new
        h[k].compare_by_identity
        h[k]
      end
      @sublevels = Hash(String, SubscriptionTree).new
      ...
      def subscribe(filter : String, session : Session, qos : UInt8)
      ...
      protected def subscribe(filter : StringTokenIterator, session : Session, qos : UInt8)
        ...
        if current == "+"
          plus = (@plus ||= SubscriptionTree.new)
          plus.subscribe filter, session, qos
          return
        end
        if !(sublevels = @sublevels[current]?)
          sublevels = @sublevels[current] = SubscriptionTree.new
        end
        sublevels.subscribe filter, session, qos
        return
      end

      def unsubscribe(filter : String, session : Session)
      ...
      protected def unsubscribe(filter : StringTokenIterator, session : Session)
        unless current = filter.next
      ...
      def each_entry(topic : String, &block : (Session, UInt8) -> _)
      ...
      protected def each_entry(topic : StringTokenIterator, &block : (Session, UInt8) -> _)
      ...
      def each_entry(&block : (Session, UInt8) -> _)
