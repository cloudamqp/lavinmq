...
module LavinMQ
  class TopicTree(TEntity)
    ...
    @leafs = Hash(String, TEntity).new

    def initialize
    end

    # Returns the replaced value or nil
    ...
    def each(filter : String, &blk : (TEntity) -> _)
      each(StringTokenIterator.new(filter, '/'), &blk)
    end

    def each(filter : StringTokenIterator, &blk : (TEntity) -> _)
      current = filter.next
      if current == "#"
        each &blk
        return
