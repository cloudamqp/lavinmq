...
module LavinMQ
  module AMQP
    class Channel < LavinMQ::Client::Channel
      ...
      getter? flow : Bool = true
      getter log : Log
      getter consumers : Array(Consumer) = Array(Consumer).new
      getter prefetch_count : UInt16 = 0_u16
      getter global_prefetch_count : UInt16 = 0_u16
      getter has_capacity : ::Channel(Nil) = ::Channel(Nil).new
      @confirm = false
      @confirm_total = 0_u64
      ...
      def details_tuple : ChannelDetails
        {
          number:                  @id,
          name:                    @name,
