...
module LavinMQ::AMQP
  class StreamQueue < DurableQueue
    class StreamS3MessageStore < StreamQueueMessageStore
      @s3_segments = Hash(UInt32, NamedTuple(path: String, etag: String, size: Int64)).new
      ...
      # TODO: Paging if there are more than 1000 segments
      private def s3_segments_from_bucket
        prefix = @msg_dir[Config.instance.data_dir.bytesize + 1..] + "/"
        path = "?delimiter=%2F&encoding-type=url&list-type=2&prefix=#{prefix}"
        h = http(URI.parse "https://#{Config.instance.streams_s3_storage_bucket}.#{Config.instance.streams_s3_storage_endpoint}")
        response = h.get(path)
        list_of_files_from_xml(XML.parse(response.body))
      ...
      def list_of_files_from_xml(body)
        ...
        @log.debug { "Found #{@s3_segments.size} segments in S3" }
      end
      ...
      private def parse_xml_element(content)
        ...
        content.children.select(&.element?).each do |element|
          ...
          when "Key"
            ...
            return nil if path.includes?(".meta")
            id = path[-10..].to_u32
          when "ETag"
        ...
        @s3_segments.each do |seg, s3file|
          ...
          spawn do
            ...
            if File.exists?("#{path}.meta")
              ...
              # upload to s3 unless it exists there?
      ...
      def download_segments
        max_running_fibers = 5 # Limit the number of concurrent downloads
        loop do
        ...
        loop do
          ...
          @downloading_segments.each do |seg_id, download_start_time|
            if @segments[seg_id]? # Segment downloaded
              ...
              @downloading_clients[seg_id]?.try &.each do |client|
                client.close # close any clients still trying to download this segment
              end
              @downloading_clients.delete(seg_id)
            elsif RoughTime.unix_ms - download_start_time > 2000 # Timeout after 2 seconds
      ...
      private def remove_local_segments
        ...
        loop do
          ...
          # If there are no current read segments (no consumers), remove the last segments
          ...
          @log.error { "Error while removing local segments: #{ex}" }
          sleep 1.seconds # Wait before trying again
          next
        end
      end
      ...
      private def http_client
        h = http(URI.parse "https://#{Config.instance.streams_s3_storage_bucket}.#{Config.instance.streams_s3_storage_endpoint}")
        h.connect_timeout = 200.milliseconds # Inactivity timeout for connecting to S3
        h.read_timeout = 500.milliseconds    # Inactivity timeout for reading from S3
        h
      end
