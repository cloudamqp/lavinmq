...
module LavinMQ::AMQP
  class Queue < LavinMQ::Queue
    ...
    def publish(msg : Message, dlx_depth : Int32 = 0) : Bool
      ...
      drop_overflow_if_no_immediate_delivery(dlx_depth)
      true
    rescue ex : MessageStore::Error
      @log.error(ex) { "Queue closed due to error" }
      close
    ...
    private def drop_overflow_if_no_immediate_delivery(dlx_depth : Int32 = 0) : Nil
      drop_overflow(dlx_depth) if (@max_length || @max_length_bytes) && !immediate_delivery?
    end

    private def drop_overflow(dlx_depth : Int32 = 0) : Nil
      counter = 0
      if ml = @max_length
        @msg_store_lock.synchronize do
          while @msg_store.size > ml
            ...
            expire_msg(env, :maxlen, dlx_depth)
            counter &+= 1
            if counter >= 16 * 1024
              Fiber.yield
              counter = 0
      ...
      if mlb = @max_length_bytes
        @msg_store_lock.synchronize do
          while @msg_store.bytesize > mlb
            env = @msg_store.shift? || break
            @log.debug { "Overflow drop head sp=#{env.segment_position}" }
            expire_msg(env, :maxlenbytes, dlx_depth)
            counter &+= 1
            if counter >= 16 * 1024
              Fiber.yield
              counter = 0
    ...
    private def expire_msg(sp : SegmentPosition, reason : Symbol, dlx_depth : Int32 = 0)
      if sp.has_dlx? || @dead_letter.dlx
        msg = @msg_store_lock.synchronize { @msg_store[sp] }
        env = Envelope.new(sp, msg, false)
        expire_msg(env, reason, dlx_depth)
      else
        delete_message sp
      end
    end

    private def expire_msg(env : Envelope, reason : Symbol, dlx_depth : Int32 = 0)
      sp = env.segment_position
      msg = env.message
      @log.debug { "Expiring #{sp} now due to #{reason}" }

      @dead_letter.route(msg, reason, dlx_depth)
