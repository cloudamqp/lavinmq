...
module LavinMQ
  module AMQP
    class TopicExchange < Exchange
      @bindings = Sync::Shared(Hash(Array(String), Set({AMQP::Destination, BindingKey}))).new(
        Hash(Array(String), Set({AMQP::Destination, BindingKey})).new do |h, k|
      ...
      def bind(destination : AMQP::Destination, routing_key, arguments = nil)
        ...
        added = @bindings.write do |bindings|
          bindings[routing_key.split(".")].add?({destination, binding_key})
        end
