  ...
  module Shovel
    class Store
      ...
      def create(name, config)
        ...
        src = source(name, config, ack_mode)
      ...
      private def source(name, config, ack_mode)
        uris = parse_uris(config["src-uri"])
        uri = uris.first
        case uri.scheme
        when "http", "https"
          Shovel::HTTPSource.new(name, uri.host || "127.0.0.1", uri.port || 0)
        else
          ...
          Shovel::AMQPSource.new(name, uris,
