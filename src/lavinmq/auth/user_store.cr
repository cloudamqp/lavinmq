  ...
  module Auth
    class UserStore
      ...
      def save!
        ...
        @replicator.try do |r|
          r.replace_file path
          # Acknowledge the user change only once a quorum has it durably, so it
          # survives a leader failover instead of living only on the leader and
          # being wiped by the new leader's full_sync. See VHostStore#save!.
