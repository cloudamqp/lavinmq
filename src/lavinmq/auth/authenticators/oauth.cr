  ...
  module Auth
    class OAuthAuthenticator < Authenticator
      ...
      def authenticate(username : String, password : Bytes) : Users::TempUser?
        if user = @users[username]? # When this authenticator is implemented it will check: if token is valid
          temp_user = Users::TempUser.new(username, user.tags, user.permissions, Time.utc + 5000.milliseconds) # get expiration time from JWT token
          return temp_user
        end
      rescue ex : Exception
