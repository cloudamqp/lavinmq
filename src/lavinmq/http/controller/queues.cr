  ...
  module HTTP
    module QueueHelpers
      private def find_queue(context, params, vhost, key = "name")
        name = params[key]
        q = vhost.queues[name]? || vhost.sessions[name]?
    ...
    class QueuesController < Controller
      ...
      private def register_routes
        get "/api/queues" do |context, _|
          itr = Iterator(Queue).chain(vhosts(user(context)).map { |v| v.queues.each_value.chain(v.sessions.each_value) })
          page(context, itr)
        end
        ...
        get "/api/queues/:vhost" do |context, params|
          with_vhost(context, params) do |vhost|
            ...
            page(context, vhost.queues.each_value.chain(vhost.sessions.each_value))
          end
        end
