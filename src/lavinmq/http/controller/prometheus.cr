      ...
      private def detailed_queue_coarse_metrics(vhosts, writer)
        ...
        vhosts.each do |vhost|
          vhost.queues.each_value do |q|
            ...
            writer.write_value("detailed_queue_messages_ready", ready, labels)
            writer.write_value("detailed_queue_messages_unacked", unacked, labels)
            writer.write_value("detailed_queue_messages", ready + unacked, labels)
        ...
        vhosts.each do |vhost|
          vhost.connections.each do |conn|
            conn.channels.each_value do |ch|
              ...
              d = ch.details_tuple
              writer.write_value("detailed_channel_consumers", d[:consumer_count], labels)
              writer.write_value("detailed_messages_unacked", d[:messages_unacknowledged], labels)
              writer.write_value("detailed_channel_prefetch", d[:prefetch_count], labels)
            end
          end
        end
      end
