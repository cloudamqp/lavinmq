...
import { UrlDataSource, DataSource, type PaginatedResponse } from './datasource.js'
...
interface QueueResponse {
  ...
  consumers: number
  consumer_details: ConsumerItem[] & { filtered_count?: number }
  state: string
  durable: boolean
  auto_delete: boolean
  exclusive: boolean
...
const pauseQueueForm = document.querySelector<HTMLFormElement>('#pauseQueue')
const resumeQueueForm = document.querySelector<HTMLFormElement>('#resumeQueue')
const restartQueueForm = document.querySelector<HTMLFormElement>('#restartQueue')
...
class ConsumersDataSource extends DataSource<ConsumerItem> {
  ...
  setConsumers(consumers: ConsumerItem[] & { filtered_count?: number }): void {
...
Table.renderTable<ConsumerItem>('table', consumersTableOpts, function (tr, item) {
...
function handleQueueState(state: string): void {
  ...
  switch (state) {
    ...
    case 'closed':
      ...
      break
    default:
      if (pauseQueueForm) (pauseQueueForm as HTMLFormElement & { disabled: boolean }).disabled = true
      if (resumeQueueForm) (resumeQueueForm as HTMLFormElement & { disabled: boolean }).disabled = true
  }
}

const chart = Chart.render('chart', 'msgs/s')
const queueUrl = HTTP.url`api/queues/${vhost}/${queue}`
function updateQueue(all: boolean): void {
  HTTP.request<QueueResponse>('GET', queueUrl + '?consumer_list_length=' + consumerListLength).then((item) => {
    if (!item) return
    ...
    if (unackEl) unackEl.textContent = String(item.messages_unacknowledged)
    const unackBytesEl = document.getElementById('q-message-bytes-unacknowledged')
    if (unackBytesEl) unackBytesEl.textContent = Helpers.nFormatter(item.message_bytes_unacknowledged) + 'B'
    const unackAvgEl = document.getElementById('q-unacked-avg-bytes')
    if (unackAvgEl) unackAvgEl.textContent = Helpers.nFormatter(item.unacked_avg_bytes) + 'B'
    const totalEl = document.getElementById('q-total')
    if (totalEl) totalEl.textContent = Helpers.formatNumber(item.messages)
    const totalBytesEl = document.getElementById('q-total-bytes')
    if (totalBytesEl) totalBytesEl.textContent = Helpers.nFormatter(item.total_bytes) + 'B'
    const totalAvgBytes =
      item.messages !== 0 ? (item.message_bytes_unacknowledged + item.message_bytes_ready) / item.messages : 0
    const totalAvgBytesEl = document.getElementById('q-total-avg-bytes')
    if (totalAvgBytesEl) totalAvgBytesEl.textContent = Helpers.nFormatter(totalAvgBytes) + 'B'
    const readyEl = document.getElementById('q-messages-ready')
    if (readyEl) readyEl.textContent = Helpers.formatNumber(item.ready)
    const readyBytesEl = document.getElementById('q-message-bytes-ready')
    if (readyBytesEl) readyBytesEl.textContent = Helpers.nFormatter(item.ready_bytes) + 'B'
    const readyAvgEl = document.getElementById('q-ready-avg-bytes')
    if (readyAvgEl) readyAvgEl.textContent = Helpers.nFormatter(item.ready_avg_bytes) + 'B'
    const consumersEl = document.getElementById('q-consumers')
    if (consumersEl) consumersEl.textContent = Helpers.formatNumber(item.consumers)
    const unackedLink = document.getElementById('unacked-link') as HTMLAnchorElement | null
    if (unackedLink) unackedLink.href = HTTP.url`/unacked#name=${queue}&vhost=${item.vhost}`
    ...
    if (all) {
      ...
      if (pagenameLabel) pagenameLabel.textContent = queue + ' in virtual host ' + item.vhost
      const queueLabel = document.querySelector('.queue')
      if (queueLabel) queueLabel.textContent = queue
      if (item.policy) {
      ...
      if (item.effective_policy_definition) {
        ...
        if (effectivePolicyEl) effectivePolicyEl.textContent = DOM.jsonToText(item.effective_policy_definition)
      }
...
const tableOptions = {
  dataSource: new UrlDataSource<BindingItem>(queueUrl + '/bindings', { useQueryState: false }),
...
if (addBindingForm) {
  addBindingForm.addEventListener('submit', function (evt) {
    ...
    const e = (data.get('source') as string).trim()
...
if (getMessagesForm) {
  getMessagesForm.addEventListener('submit', function (evt) {
    ...
    const body = {
      count: parseInt(data.get('messages') as string, 10),
      ack_mode: data.get('mode'),
      encoding: data.get('encoding'),
      truncate: 50000,
    }
    HTTP.request<QueueMessage[]>('POST', url, { body }).then((messages) => {
      ...
      for (const message of messages) {
        const msgNode = template.cloneNode(true) as HTMLElement
        msgNode.removeAttribute('id')
        const numberEl = msgNode.querySelector('.message-number')
        if (numberEl) numberEl.textContent = String(messages.indexOf(message) + 1)
        const remainingEl = msgNode.querySelector('.messages-remaining')
        if (remainingEl) remainingEl.textContent = String(message.message_count)
        ...
        if (routingKeyEl) routingKeyEl.textContent = message.routing_key
        const redeliveredEl = msgNode.querySelector('.message-redelivered')
        if (redeliveredEl) redeliveredEl.textContent = String(message.redelivered)
        const propsEl = msgNode.querySelector('.message-properties')
        if (propsEl) propsEl.textContent = JSON.stringify(message.properties)
        const sizeEl = msgNode.querySelector('.message-size')
        if (sizeEl) sizeEl.textContent = String(message.payload_bytes)
        const encodingEl = msgNode.querySelector('.message-encoding')
        if (encodingEl) encodingEl.textContent = message.payload_encoding
        const payloadEl = msgNode.querySelector('.message-payload')
        if (payloadEl) payloadEl.textContent = message.payload
...
if (moveMessagesForm) {
  moveMessagesForm.addEventListener('submit', function (evt) {
    ...
    const destInput = document.querySelector<HTMLInputElement>('[name=shovel-destination]')
...
if (purgeForm) {
  purgeForm.addEventListener('submit', function (evt) {
    ...
    const form = evt.target as HTMLFormElement
    let params = ''
    const countElem = form.querySelector<HTMLInputElement>("input[name='count']")
...
if (restartQueueForm) {
  restartQueueForm.addEventListener('submit', function (evt) {
    ...
    if (window.confirm('Are you sure? This will restart the queue.')) {
      HTTP.request<RestartResponse>('PUT', url).then((res) => {
