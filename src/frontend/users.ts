...
import type { TableController } from './table.js'
...
let usersTable: TableController | null = null
HTTP.request<Permission[]>('GET', 'api/permissions')
  .then((permissions) => {
    const tableOptions = {
      url: 'api/users',
    ...
    usersTable = Table.renderTable<UserItem>('users', tableOptions, (tr, item, all) => {
      if (all) {
  ...
  createForm.addEventListener('submit', function (evt) {
    ...
    trs.forEach((tr) => {
      if (username === tr.getAttribute('data-name')) {
        ...
        if (data.get('tags') === '') {
          const tagsCell = tr.childNodes[1] as HTMLElement | null
          data.set('tags', tagsCell?.textContent ?? '')
        }
      }
    })
    const body: { tags: FormDataEntryValue | null; password?: FormDataEntryValue | null } = {
      tags: data.get('tags'),
    }
...
function generatePassword(): void {
  ...
  const input = document.querySelector<HTMLInputElement>('#createUser input[name="password"]')
  if (input) {
...
function togglePasswordAsPlainText(): void {
  const input = document.querySelector<HTMLInputElement>('#createUser input[name="password"]')
  if (input) {
