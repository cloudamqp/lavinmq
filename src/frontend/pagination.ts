import type { DataSource } from './datasource.js'

interface MakeLinkOptions {
  class?: string
  disabled?: boolean
}

class Pagination<T> {
  #container: HTMLElement
  #dataSource: DataSource<T>

  constructor(container: HTMLElement, dataSource: DataSource<T>) {
    this.#container = container
    this.#dataSource = dataSource
    this.#dataSource.on('update', this.update.bind(this))
  }

  #makeLink(toPage: number, label: string | number = toPage, opts: MakeLinkOptions = {}): HTMLDivElement {
    const div = document.createElement('div')
    div.classList.add('page-item')
    if (opts.class && opts.class.length > 0) {
      div.classList.add(...opts.class.split(' '))
    }
    let tag: 'a' | 'span' = 'a'
    if (opts.disabled === true) {
      div.classList.add('disabled')
      tag = 'span'
    }
    const button = document.createElement(tag)
    button.textContent = String(label)
    const params = this.#dataSource.queryParams()
    params.set('page', String(toPage))
    div.appendChild(button)
    if (opts.disabled !== true && button instanceof HTMLAnchorElement) {
      button.href = `#${params.toString()}`
      button.addEventListener('click', (e) => {
        e.preventDefault()
        this.#dataSource.page = toPage
        this.#dataSource.reload()
        return false
      })
    }
    return div
  }

  update(): void {
    let active: string
    const page = this.#dataSource.page
    const pages = this.#dataSource.pageCount
    let pageCutLow = page - 1
    let pageCutHigh = page + 1
    if (pages <= 1) {
      this.#container.style.display = 'none'
      return
    }
    this.#container.style.display = ''

    const links: HTMLDivElement[] = []

    const prevLink = this.#makeLink(page - 1, 'Previous', { class: 'previous', disabled: page === 1 })
    links.push(prevLink)
    if (pages < 6) {
      for (let p = 1; p <= pages; p++) {
        active = page === p ? 'active' : ''
        links.push(this.#makeLink(p, p, { class: active }))
      }
    } else {
      if (page === 1) {
        pageCutHigh += 2
      } else if (page === 2) {
        pageCutHigh += 1
      }
      if (page === pages) {
        pageCutLow -= 2
      } else if (page === pages - 1) {
        pageCutLow -= 1
      }
      if (page > 2) {
        links.push(this.#makeLink(1))
        if (page > 3) {
          links.push(this.#makeLink(2, '...', { class: 'out-of-range' }))
        }
      }
      for (let p = pageCutLow; p <= pageCutHigh; p++) {
        if (p === 0) {
          p += 1
        }
        if (p > pages) {
          continue
        }
        active = page === p ? 'active' : ''
        links.push(this.#makeLink(p, p, { class: active }))
      }
      if (page < pages - 1) {
        if (page < pages - 2) {
          links.push(this.#makeLink(pages - 1, '...', { class: 'out-of-range' }))
        }
        links.push(this.#makeLink(pages))
      }
    }
    const nextLink = this.#makeLink(page + 1, 'Next', { class: 'next', disabled: page === pages })
    links.push(nextLink)
    while (this.#container.firstChild) this.#container.removeChild(this.#container.firstChild)
    links.forEach((l) => this.#container.appendChild(l))
  }
}

export function create<T>(container: HTMLElement, dataSource: DataSource<T>): Pagination<T> {
  return new Pagination(container, dataSource)
}
