...
const queueDataSource = new UrlDataSource<QueueItem>(url)
const tableOptions = {
...
const performMultiAction = (el: Event): void => {
  const target = el.target as HTMLElement
  const action = target.dataset.action
  const elems = document.querySelectorAll<HTMLInputElement>('input[data-name]:checked')
    ...
    HTTP.request('DELETE', actionUrl)
      .then(() => {
        ...
        if (performed === totalCount) {
          ...
          const checkAll = document.getElementById('multi-check-all') as HTMLInputElement | null
...
const toggleMultiActionControls = (show: boolean, count: number): void => {
...
if (multiCheckAll) {
  multiCheckAll.addEventListener('change', (el) => {
    ...
    let c = 0
    document.querySelectorAll<HTMLInputElement>('input[data-name]').forEach((elem) => {
...
const queuesTable = Table.renderTable<QueueItem>('table', tableOptions, function (tr, item, all) {
  if (all) {
    if (item.internal) {
...
if (declareForm) {
  declareForm.addEventListener('submit', function (evt) {
    ...
    const queue = (data.get('name') as string).trim()
    ...
    HTTP.request<CreateQueueResponse>('PUT', createUrl, { body }).then((response) => {
      if (response?.is_error) {
        return
      }
      queuesTable.reload()
      form.reset()
      const vhostSelect = form.querySelector<HTMLSelectElement>('select[name="vhost"]')
      if (vhostSelect) vhostSelect.value = decodeURIComponent(formVhost)
