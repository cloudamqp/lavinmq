...
function updateUser(): void {
  ...
  HTTP.request<UserResponse>('GET', userUrl).then((item) => {
    if (!item) return
    ...
    if (hasPasswordEl) hasPasswordEl.textContent = hasPassword
    tagHelper(item.tags)
  })
}

function tagHelper(tags: string): void {
  const vals = tags.split(',')
  vals.forEach((val) => {
    const input = document.querySelector<HTMLInputElement>('[name=tags]')
    if (input) {
...
const permissionsTable = Table.renderTable<PermissionItem>('permissions', tableOptions, (tr, item, all) => {
  ...
  if (all) {
    ...
    const editBtn = DOM.button.edit({
      click: function () {
        Form.editItem<PermissionItem>('#setPermission', item)
      },
    })
...
if (setPermForm) {
  setPermForm.addEventListener('submit', function (evt) {
    ...
    const vhost = data.get('vhost') as string
...
const removePasswordCheckbox = document.querySelector<HTMLInputElement>('[name=remove_password]')
if (removePasswordCheckbox) {
  removePasswordCheckbox.addEventListener('change', function () {
    const pwd = document.querySelector<HTMLInputElement>('[name=password]')
    if (pwd) {
      if (this.checked) {
        pwd.disabled = true
        pwd.required = false
...
if (updateForm) {
  updateForm.addEventListener('submit', function (evt) {
    ...
    const body: { tags: FormDataEntryValue | null; password_hash?: string; password?: FormDataEntryValue | null } = {
      tags: data.get('tags'),
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (pagenameLabel) pagenameLabel.textContent = user
  updateUser()
})
