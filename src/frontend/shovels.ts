...
import { UrlDataSource, type PaginatedResponse } from './datasource.js'

interface ShovelValue {
  'src-uri': string | string[]
  'dest-uri': string | string[]
  'src-prefetch-count'?: number
  'src-delete-after'?: string
  ...
  'src-consumer-args'?: Record<string, string | number>
}

interface ShovelItem {
  vhost: string
  name: string
  value: ShovelValue
  state?: string
...
function renderState(item: ShovelItem): HTMLElement | string {
  if (item.error) {
...
class ShovelsDataSource extends UrlDataSource<ShovelItem> {
  statusUrl: string

  constructor(baseUrl: string, shovelsStatusUrl: string) {
    super(baseUrl)
    this.statusUrl = shovelsStatusUrl
  }

  protected override _reload(): Promise<PaginatedResponse<ShovelItem> | ShovelItem[]> {
    const p1 = super._reload()
    const p2 = HTTP.request<ShovelStatus[]>('GET', this.statusUrl)

    return Promise.all([p1, p2]).then((values) => {
      const response = values[0]
      let shovels: ShovelItem[] = 'items' in response ? response.items : response
...
Table.renderTable<ShovelItem>('table', tableOptions, (tr, item) => {
  ...
  if (Array.isArray(srcUri)) {
    Table.renderCell(
      tr,
      2,
      srcUri.map((uri) => decodeURI(uri.replace(/:([^:]+)@/, ':***@'))).join(', ')
    )
  } else {
  ...
  let srcType: 'exchange' | 'stream' | 'queue' = 'exchange'
  ...
  if (Array.isArray(destUri)) {
    Table.renderCell(
      tr,
      5,
      destUri.map((uri) => decodeURI(uri.replace(/:([^:]+)@/, ':***@'))).join(', ')
    )
  } else {
  ...
  const editBtn = DOM.button.edit({
    click: function () {
      Form.editItem<ShovelItem>('#createShovel', item, {
        'src-type': () => srcType,
        ...
        'src-offset': () => (consumerArgs['x-stream-offset'] as string | undefined),
      })
    },
  })
...
const srcTypeSelect = document.querySelector<HTMLSelectElement>('[name=src-type]')
if (srcTypeSelect) {
...
const destUriInput = document.querySelector<HTMLInputElement>('[name=dest-uri]')
if (destUriInput) {
...
if (createForm) {
  createForm.addEventListener('submit', function (evt) {
    ...
    const body: { value: ShovelValue } = {
      value: {
        'src-uri': data.get('src-uri') as string,
        'dest-uri': data.get('dest-uri') as string,
        'src-prefetch-count': parseInt(data.get('src-prefetch-count') as string, 10),
        'src-delete-after': data.get('src-delete-after') as string,
        'reconnect-delay': parseInt(data.get('reconnect-delay') as string, 10),
        'ack-mode': data.get('ack-mode') as string,
      },
    }
    const srcType = data.get('src-type')
    const offset = data.get('src-offset') as string
    switch (srcType) {
      case 'queue':
        body.value['src-queue'] = data.get('src-endpoint') as string
        break
      case 'exchange':
        body.value['src-exchange'] = data.get('src-endpoint') as string
        body.value['src-exchange-key'] = data.get('src-exchange-key') as string
        break
      case 'stream':
        body.value['src-queue'] = data.get('src-endpoint') as string
        if (offset.length) {
          const args: Record<string, string | number> = body.value['src-consumer-args'] || {}
          if (/^\d+$/.test(offset)) {
    ...
    if (data.get('dest-type') === 'queue') {
      body.value['dest-queue'] = data.get('dest-endpoint') as string
    } else {
      body.value['dest-exchange'] = data.get('dest-endpoint') as string
    }
