...
import { UrlDataSource, type DataSource, type PaginatedResponse } from './datasource.js'
...
export function renderTable<T>(
  ...
  const dataSource = options.dataSource ?? new UrlDataSource<T>(options.url ?? '')
  ...
  const tableHeader = grandparent ? grandparent.querySelector<HTMLElement>(':scope > .table-header') : null
  ...
  dataSource.on('error', ((error: CustomEvent) => {
    console.log(error)
    toggleDisplayError(id, 'Error fetching data: ' + error.detail)
  }) as EventListener)
  dataSource.reload()

  function on(event: string, args: EventListenerOrEventListenerObject): void {
    events.addEventListener(event, args)
  }

  function reload(): Promise<PaginatedResponse<T> | T[] | void> {
  ...
  function updateTable(): void {
    ...
    const tableEl = document.getElementById(id) as HTMLTableElement | null
    if (!tableEl) return
    ...
    if (!Array.isArray(items) || items.length === 0) {
      ...
      td.colSpan = headerRow
        ? Array.from(headerRow.children).reduce((sum, item) => sum + (item as HTMLTableCellElement).colSpan, 0)
        : 1
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        toggleDisplayError(id, (item as { error?: string }).error || error.message)
      }
    }
  ...
  function findRow(rows: HTMLTableRowElement[], item: T): HTMLTableRowElement | undefined {
    return rows.find((row) => keyColumns.every((key) => row.dataset[key] === JSON.stringify((item as Record<string, unknown>)[key])))
  }

  function setKeyAttributes(tr: HTMLTableRowElement, item: T): void {
    keyColumns.forEach((key) => {
      tr.dataset[key] = JSON.stringify((item as Record<string, unknown>)[key])
    })
  }

  function renderSearch(cont: HTMLElement, ds: DataSource<T>): void {
    ...
    if (cont.dataset['clearSearch'] === '1') ds.searchTerm = ''
    filterInput.value = ds.searchTerm ?? ''
    form.appendChild(filterInput)
    cont.insertBefore(form, cont.children[0] ?? null)

    let liveType: ReturnType<typeof setTimeout>
    const apply = (): void => {
...
export function renderCell(
  ...
  column: number,
  value: Element | string | number | null | undefined,
  classList = ''
  ...
  if (cell.classList.contains('hide')) return undefined
  cell.className = classList
  return cell
}

function buildCells(tr: HTMLTableRowElement, index: number): HTMLTableCellElement | undefined {
  ...
  const tbl = tr.parentElement?.parentElement as HTMLTableElement | null
...
function columnSelectorCacheKey(table: HTMLTableElement): string {
  return `hiddenTableCols${window.location.pathname}${table.id}`
}

function setHiddenColumns(table: HTMLTableElement, state: Set<number>): void {
...
function getHiddenColumns(table: HTMLTableElement): Set<number> {
  const cacheKey = columnSelectorCacheKey(table)
  return new Set(JSON.parse(window.sessionStorage.getItem(cacheKey) || '[]') as number[])
}

function toggleCol(table: HTMLTableElement, colIndex: number): void {
...
function renderColumnSelector(table: HTMLTableElement): void {
  ...
  container.addEventListener('click', (e) => {
    ...
    if (!target.classList.contains('col-toggle')) return
    ...
    container.parentElement?.addEventListener('click', (ev) => {
      ...
      if (tgt.classList.contains('close')) close()
    })
    container.parentElement?.addEventListener('change', (ev) => {
      const tgt = ev.target as HTMLInputElement
      if (!tgt.classList.contains('col-toggle-checkbox')) return
...
export function toggleDisplayError(tableID: string, message: string | null): void {
