...
function updateQueue(all: boolean): void {
  HTTP.request<StreamQueueResponse>('GET', queueUrl + '?consumer_list_length=' + consumerListLength).then((item) => {
    if (!item) return
...
if (addBindingForm) {
  addBindingForm.addEventListener('submit', function (evt) {
    ...
    HTTP.request('POST', url, { body }).then(() => {
...
if (getMessagesForm) {
  getMessagesForm.addEventListener('submit', function (evt) {
    ...
    HTTP.request<StreamMessage[]>('POST', url, { body }).then((messages) => {
      if (!messages) return
      ...
      for (const message of messages) {
        ...
        if (numberEl) numberEl.textContent = String(offset)
