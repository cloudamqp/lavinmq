...
A shovel with a single destination never fails over: an `Abort` there counts straight towards the shovel's abort threshold, on the same connection, since reconnecting does not fix an unusable endpoint.
