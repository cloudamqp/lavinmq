...
- If no data is received within the heartbeat timeout plus 5 seconds, the connection is closed
...
|-----------|---------|-------------|
| `unix_proxy_protocol` (in `[amqp]`) | `1` | PROXY protocol version on Unix sockets, applies to all protocols (0 = disabled) |
| `tcp_proxy_protocol` (in `[amqp]`) | `0` | PROXY protocol version on TCP, applies to AMQP and MQTT (0 = disabled) |
...
| `tcp_keepalive` | `60:10:3` | Idle, interval, probes (colon-separated) |
...
| `unix_path` (in `[mqtt]`) | MQTT Unix socket path |
| `unix_path` (in `[mgmt]`) | HTTP Unix socket path |
