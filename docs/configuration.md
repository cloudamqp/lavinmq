...
| `topic_permissions` | — | — | Bool | `false` | Enable per-topic MQTT publish/receive authorization via permission groups. When enabled, topic rules govern access (default-deny) and replace the coarse `permission_check_enabled` check for MQTT. |
