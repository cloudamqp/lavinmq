...
describe LavinMQ::Auth::LocalUser do
  describe "#update_password" do
    it "should not update password hash when given same password as current" do
      u = LavinMQ::Auth::LocalUser.create("username", "password", "sha256", [] of LavinMQ::Tag)
  ...
  describe "Permission cache" do
    ...
    it "should actually use cached results (returns stale data until revision changes)" do
      user = LavinMQ::Auth::LocalUser.create("username", "password", "sha256", [] of LavinMQ::Tag)
...
describe LavinMQ::Server do
  ...
  it "allows changing default user" do
    ...
    LavinMQ::Config.instance.default_password = LavinMQ::Auth::LocalUser.hash_password("spec", "SHA256").to_s
    with_amqp_server do |s|
      with_channel(s, user: "spec", password: "spec") { }
    end
  ensure
    LavinMQ::Config.instance.default_user = "guest"
    LavinMQ::Config.instance.default_password = LavinMQ::Auth::LocalUser.hash_password("guest", "SHA256").to_s
  end
