  ...
  describe "AMQP" do
    ...
    it "should shovel large messages" do
      with_amqp_server do |s|
        ...
        dest = LavinMQ::Shovel::AMQPDestination.new("spec", URI.parse(amqp(s).url), "lm_q2", direct_user: s.users.direct_user)
    ...
    it "should shovel forever" do
      with_amqp_server do |s|
        vhost = s.vhosts.create("x")
        source = LavinMQ::Shovel::AMQPSource.new("spec", [URI.parse(amqp(s).url)], "sf_q1", direct_user: s.users.direct_user)
        dest = LavinMQ::Shovel::AMQPDestination.new("spec", URI.parse(amqp(s).url), "sf_q2", direct_user: s.users.direct_user)
    ...
    it "should shovel over amqps" do
      with_amqp_server(tls: true) do |s|
        ...
        source = LavinMQ::Shovel::AMQPSource.new(
          "spec",
          [URI.parse("#{amqp(s).url.sub("amqp://", "amqps://")}?verify=none")],
          "ssl_q1",
          direct_user: s.users.direct_user
        )
        dest = LavinMQ::Shovel::AMQPDestination.new(
