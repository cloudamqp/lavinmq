...
describe AvalancheMQ::VHost do
  ...
  it "should be able to add policy" do
    vhost.add_policy("test", definitions, "^.*$", "all", -10_i8)
  ...
  it "should be able to list policies" do
    ...
    vhost2.add_policy("test", definitions, " ^.*$", "all", -10_i8)
  ...
  it "should overwrite policy with same name" do
    vhost.add_policy("test", definitions, "^.*$", "all", -10_i8)
    vhost.add_policy("test", definitions, "^.*$", "exchanges", 10_i8)
    vhost.policies.size.should eq 1
  ...
  it "should validate pattern" do
    expect_raises(ArgumentError) do
      vhost.add_policy("test", definitions, "(bad", "all", -10_i8)
    end
    vhost.delete_policy("test")
  end
  ...
  it "should validate appy_to" do
    expect_raises(ArgumentError) do
      vhost.add_policy("test", definitions, "^.*$", "bad", -10_i8)
    end
    vhost.delete_policy("test")
  end
  ...
  it "should apply policy" do
    ...
    vhost.add_policy("ml", definitions, "^.*$", "queues", 11_i8)
    Fiber.yield
  ...
  it "should respect priroty" do
    ...
    vhost.add_policy("ml2", definitions, "^.*$", "queues", 1_i8)
    vhost.add_policy("ml1", definitions, "^.*$", "queues", 0_i8)
    Fiber.yield
