...
describe LavinMQ::Config do
  ...
  it "can set tls_cert and tls_key in amqp section" do
    config_file = File.tempfile do |file|
      file.print <<-CONFIG
        [amqp]
        tls_cert = /etc/amqp/cert.pem
        tls_key = /etc/amqp/key.pem
      CONFIG
    end
    config = LavinMQ::Config.new
    argv = ["-c", config_file.path]
    config.parse(argv)
    config.tls_cert_path.should eq "/etc/amqp/cert.pem"
    config.tls_key_path.should eq "/etc/amqp/key.pem"
  end

  it "can set tls_cert and tls_key in mgmt section" do
    config_file = File.tempfile do |file|
      file.print <<-CONFIG
        [mgmt]
        tls_cert = /etc/mgmt/cert.pem
        tls_key = /etc/mgmt/key.pem
      CONFIG
    end
    config = LavinMQ::Config.new
    argv = ["-c", config_file.path]
    config.parse(argv)
    config.tls_cert_path.should eq "/etc/mgmt/cert.pem"
    config.tls_key_path.should eq "/etc/mgmt/key.pem"
  end

  it "main section tls_cert takes precedence over amqp section (main after amqp)" do
    config_file = File.tempfile do |file|
      file.print <<-CONFIG
        [amqp]
        tls_cert = /etc/amqp/cert.pem
        [main]
        tls_cert = /etc/main/cert.pem
      CONFIG
    end
    config = LavinMQ::Config.new
    argv = ["-c", config_file.path]
    config.parse(argv)
    config.tls_cert_path.should eq "/etc/main/cert.pem"
  end

  it "main section tls_cert takes precedence over amqp section (main before amqp)" do
    config_file = File.tempfile do |file|
      file.print <<-CONFIG
        [main]
        tls_cert = /etc/main/cert.pem
  ...
  it "warns when tls_cert is set in multiple sections" do
    config_file = File.tempfile do |file|
      file.print <<-CONFIG
        [main]
        tls_cert = /etc/main/cert.pem
    ...
    Log.capture(level: :warn) do |logs|
      ...
      config.parse(argv)
      config.tls_cert_path.should eq "/etc/main/cert.pem"
      logs.check(:warn, /tls_cert in \[amqp\] ignored, already set/)
    end
  end

  it "warns when tls_cert is set in both amqp and mgmt sections" do
    config_file = File.tempfile do |file|
      file.print <<-CONFIG
        [amqp]
        tls_cert = /etc/amqp/cert.pem
    ...
    Log.capture(level: :warn) do |logs|
      ...
      config.parse(argv)
      config.tls_cert_path.should eq "/etc/amqp/cert.pem"
      logs.check(:warn, /tls_cert in \[mgmt\] ignored, already set/)
    end
  end
end
