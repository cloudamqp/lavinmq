...
# Mirror the launcher's SNI callback (reads config.sni_manager on each
# handshake) and return the CN of the certificate served for *hostname*.
private def sni_served_cn(config : LavinMQ::Config, hostname : String) : String?
  default_ctx = OpenSSL::SSL::Context::Server.new
  default_ctx.certificate_chain = "spec/resources/server_certificate.pem"
  default_ctx.private_key = "spec/resources/server_key.pem"
  default_ctx.on_server_name do |name|
    config.sni_manager.get_host(name).try(&.amqp_tls_context)
  end
  tcp_server = TCPServer.new("127.0.0.1", 0)
  port = tcp_server.local_address.port
  spawn do
    if client = tcp_server.accept?
      begin
        OpenSSL::SSL::Socket::Server.new(client, default_ctx, sync_close: true).close
      rescue
  ...
  client_ctx = OpenSSL::SSL::Context::Client.new
  client_ctx.verify_mode = OpenSSL::SSL::VerifyMode::NONE
  ssl_client = OpenSSL::SSL::Socket::Client.new(tcp_client, client_ctx, hostname: hostname)
...
describe "SNI certificate reload" do
  it "serves a rotated certificate for an existing SNI host after reload" do
    ...
    begin
      ...
      sni_served_cn(config, "foobar.localhost").should eq "foobar.localhost"

      # Point the same host at a different certificate and reload. The new cert
      # must be served even though nothing explicitly invalidates the cached
      # context - reload rebuilds the SNIHost from scratch.
      File.write(config_file.path, <<-INI)
      [sni:foobar.localhost]
      tls_cert = spec/resources/server_certificate.pem
      tls_key = spec/resources/server_key.pem
      INI
      config.reload
      sni_served_cn(config, "foobar.localhost").should eq "anders"
    ensure
